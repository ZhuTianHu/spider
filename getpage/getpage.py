...
def _get_page_requests(url, proxy = '', fpfirst = False, headers = None,
    ...
    try:
        ...
        valid_url = True
        rargs['stream'] = True
        with closing(session.get(url, **rargs)) as resp:
            for line in resp.iter_content(chunk_size=512):
                content += line
                length += len(line)
                if content_length_limit and length > content_length_limit:
                    valid_url = False
                    break 
        if not valid_url:
            LOG.warning('Content length was %s, longger than limit %s'
...
def get(url, retry = 3, fpfirst = False, use_proxy = False, render_js = False,
        headers = None, content_length_limit = None):
    """
...
if __name__ == '__main__':
    ...
    if p:
        ...
        print 'text', p.text[:1000], repr(p.text[:1000])
        print 'text type', type(p.text)
        print 'content', p.content[:1000], repr(p.content[:1000])
        print 'content type', type(p.content)
